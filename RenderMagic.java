...
public class RenderMagic extends Render
{
    ...
    /** The RNG used in RenderMagic (for bobbing magictacks on the ground) */
    private Random random;
    public boolean field_27004_a;

    /** Defines the zLevel of rendering of magic on GUI. */
    public float zLevel;
     ...
     * Renders the magic
     */
    public void doRenderMagic(EntityMagic par1EntityMagic, double par2, double par4, double par6, float par8, float par9)
    {
        random.setSeed(187L);
        MagicStack magictack = par1EntityMagic.magic;
        GL11.glPushMatrix();
        float f = MathHelper.sin(((float)par1EntityMagic.age + par9) / 10F + par1EntityMagic.field_804_d) * 0.1F + 0.1F;
        float f1 = (((float)par1EntityMagic.age + par9) / 20F + par1EntityMagic.field_804_d) * (180F / (float)Math.PI);
        byte byte0 = 1;

        if (par1EntityMagic.magic.stackSize > 1)
        {
            byte0 = 2;
        }

        if (par1EntityMagic.magic.stackSize > 5)
        {
            byte0 = 3;
        }

        if (par1EntityMagic.magic.stackSize > 20)
        {
            byte0 = 4;
        }
        ...
        if (magictack.magicID < 256 && RenderBlocks.renderItemIn3d(Block.blocksList[magictack.magicID].getRenderType())) //MAY COUSE CRASH!
        {
            GL11.glRotatef(f1, 0.0F, 1.0F, 0.0F);
            loadTexture("/terrain.png");
            float f2 = 0.25F;
            int k = Block.blocksList[magictack.magicID].getRenderType();
            ...
            for (int j1 = 0; j1 < byte0; j1++)
            {
                ...
                float f6 = 1.0F;
                renderBlocks.renderBlockAsItem(Block.blocksList[magictack.magicID], magictack.getMagicDamage(), f6);
                GL11.glPopMatrix();
            }
        }
        else if (magictack.getMagic().func_46058_c())
        {
            ...
            for (int i = 0; i <= 1; i++)
            {
                int l = magictack.getMagic().func_46057_a(magictack.getMagicDamage(), i);
                float f3 = 1.0F;

                if (field_27004_a)
                {
                    int k1 = Magic.magicList[magictack.magicID].getColorFromDamage(magictack.getMagicDamage(), i);
        ...
        {
            GL11.glScalef(0.5F, 0.5F, 0.5F);
            int j = magictack.getIconIndex();

            if (magictack.magicID < 256)
            {
                loadTexture("/terrain.png");
            }
            else
            ...
            if (field_27004_a)
            {
                int i1 = Magic.magicList[magictack.magicID].getColorFromDamage(magictack.getMagicDamage(), 0);
    ...
    public void drawMagicIntoGui(FontRenderer par1FontRenderer, RenderEngine par2RenderEngine, int par3, int par4, int par5, int par6, int par7)
    {
        if (par3 < 256 && RenderBlocks.renderItemIn3d(Block.blocksList[par3].getRenderType()))
        {
            ...
            int i1 = Magic.magicList[par3].getColorFromDamage(par4, 0);
        ...
        else if (Magic.magicList[par3].func_46058_c())
        {
            ...
            for (int j = 0; j <= 1; j++)
            {
                int l = Magic.magicList[par3].func_46057_a(par4, j);
                int j1 = Magic.magicList[par3].getColorFromDamage(par4, j);
        ...
        else if (par5 >= 0)
        {
            ...
            int k = Magic.magicList[par3].getColorFromDamage(par4, 0);
    ...
    /**
     * Renders the magic's icon or block into the UI at the specified position.
     */
    public void renderMagicIntoGUI(FontRenderer par1FontRenderer, RenderEngine par2RenderEngine, MagicStack par3MagicStack, int par4, int par5)
    {
        if (par3MagicStack == null)
        {
            return;
        }

        drawMagicIntoGui(par1FontRenderer, par2RenderEngine, par3MagicStack.magicID, par3MagicStack.getMagicDamage(), par3MagicStack.getIconIndex(), par4, par5);

        if (par3MagicStack != null && par3MagicStack.hasEffect())
        {
    ...
    /**
     * Renders the magic's overlay information. Examples being stack count or damage on top of the magic's image at the
     * specified position.
     */
    public void renderMagicOverlayIntoGUI(FontRenderer par1FontRenderer, RenderEngine par2RenderEngine, MagicStack par3MagicStack, int par4, int par5)
    {
        if (par3MagicStack == null)
        {
            return;
        ...
        if (par3MagicStack.stackSize > 1)
        {
            String s = (new StringBuilder()).append("").append(par3MagicStack.stackSize).toString();
            GL11.glDisable(GL11.GL_LIGHTING);
        ...
        if (par3MagicStack.isMagicDamaged())
        {
            int i = (int)Math.round(13D - ((double)par3MagicStack.getMagicDamageForDisplay() * 13D) / (double)par3MagicStack.getMaxDamage());
            int j = (int)Math.round(255D - ((double)par3MagicStack.getMagicDamageForDisplay() * 255D) / (double)par3MagicStack.getMaxDamage());
            GL11.glDisable(GL11.GL_LIGHTING);
            GL11.glDisable(GL11.GL_DEPTH_TEST);
    ...
    {
        doRenderMagic((EntityMagic)par1Entity, par2, par4, par6, par8, par9);
    }
}
